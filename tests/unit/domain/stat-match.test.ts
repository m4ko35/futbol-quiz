...
describe("officialTotal — BR-23", () => {
  it("iki parça da doluysa toplar", () => {
    expect(officialTotal(400, 50)).toBe(450);
    expect(officialTotal(0, 0)).toBe(0);
  });

  it("kulüp yarısı null ise toplam null", () => {
    expect(officialTotal(null, 50)).toBeNull();
  });

  it("millî yarısı null ise toplam null (belirsizlik yayılır)", () => {
    expect(officialTotal(400, null)).toBeNull();
  });
});
