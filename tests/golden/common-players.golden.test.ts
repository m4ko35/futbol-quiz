 ...
 *
 * SÜRE SINIRI DOSYAYA ÖZGÜ ve ölçülerek konuldu (21 Ağustos 2026). Bu dosya
 * 169 MB'lık gerçek `dev.db`'ye Prisma üzerinden vuruyor; 89 test dosyası
 * paralel koşarken disk için yarışıyor. Ölçüm:
 *
 *   tek başına (en yavaş test)     171 ms
 *   tam koşu altında, geçen koşu   662 ms
 *   tam koşu altında, düşen koşu  >5000 ms  ← varsayılan `testTimeout`
 *
 * Yani iddialar değil G/Ç yarışı düşüyordu ve kapı, kendi ölçülmemiş
 * varsayımı yüzünden aralıklı olarak kırmızı veriyordu. 20 saniye, yük
 * altında GÖZLENEN en kötü değerin otuz katı; sınır kaldırılmadı, ölçüye
 * oturtuldu. Aynı gerekçe `vitest.config.ts`'teki `hookTimeout` için de
 * yazılı — orada kurulum, burada sorgular.
 */
