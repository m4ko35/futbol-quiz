...
#### Aşama 3 — Kadro keşfi (§4.7'nin bıraktığı boşluk)

**§4.7'nin "%26 eksik" sayısı ölçümle çürüdü ve sebebi örneklemdi.** O sayı sekiz seçkin kulüpten geliyordu. 22 Ağustos 2026'da tam sayım yapıldı: aktif kadrosu olan **889 kulübün** tamamı bağımsız kaynaktan (Vikipedi kadro şablonları) okundu, 877'sinin İngilizce makalesi var, **730'unda** `{{fs start}}` ailesinden bir kadro bloğu bulundu. Karşılaştırılan kadro yeri: **19.666**.

| Sınıf | Anlamı                                         |   Yer | Pay       |
| ----- | ---------------------------------------------- | ----: | --------- |
| **A** | Veri kümemizde var                             | 7.473 | %38,0     |
| **B** | Wikidata ögesi **var**, veri kümemizde **yok** | 5.615 | **%28,6** |
| C     | Makalesi var, Wikidata ögesi yok               |   409 | %2,1      |
| D     | Hiç Vikipedi makalesi yok                      | 6.169 | %31,4     |

**Boşluk tek parça değil ve yalnızca biri kusur.** D sınıfına hiçbir yöntem ulaşamaz — makalesi olmayan oyuncu zaten tanınırlık havuzuna da giremez. Asıl boşluk **B: 5.547 tekil oyuncu**. Eski ölçümle kıyaslanabilir taban (QID'si olanlar) **%57,1** kapsam veriyor: eksiklik %26 değil **%43**.

Sekiz kulüpten ölçmenin neden yanılttığı da görüldü — dağılım geniş: en büyük 40 kulüpte kapsam **%66,0**, ama Athletic Bilbao %97,0 ile Ajax %48 arasında oynuyor. Kulüp büyüdükçe kapsam artıyor, ligle ilgisi yok:

| Kulüp bandı (veri kümesindeki dönem) | Kapsam | D (makalesiz) |
| ------------------------------------ | -----: | ------------: |
| En büyük 40                          |  %66,0 |          %2,5 |
| 500+ dönem (286 kulüp)               |  %47,9 |         %17,9 |
| 200–499                              |  %32,9 |         %37,6 |
| 200'den az                           |  %28,0 |         %49,5 |

**Sebep ölçüldü: boru hattında kusur YOK.** 5.547 oyuncunun `P54` ifadeleri tek tek soruldu ve **hiçbirinde evrendeki bir kulübe bağ bulunmadı** — yani tek bir kayıt bile "Wikidata söylüyordu, biz kaçırdık" sınıfında değil. %81,9'unun hiç `P54` ifadesi yok, %18,1'inin bağları yalnızca kapsam dışı kulüplere. §10.2'nin "yapısal" teşhisi doğrulandı.

> Wikidata'da ögenin **var olması** ile kulüp bağının var olması ayrı şeylerdir ve bu ayrım kolayca karıştırılıyor. Leny Yoro'nun ögesi (`Q112039295`) 33 dilde makaleye sahip; `P54` sayısı **sıfır**. Aynı durum Pape Matar Sarr, Josip Šutalo ve Renato Veiga'da da ölçüldü. Matteo Gabbia'nın iki `P54` ifadesi var ve ikisi de kullanılamaz: biri İtalya U-20, öteki bir **anlam ayrımı sayfası** (`Q28858993`).

**Eksiklerin profili bir yaş kusuru.** %77,8'i 2000 ve sonrası doğumlu, %22,7'si 2005+; 1994 ve öncesi yalnızca %3,4. Mevki dağılımı düz. Wikidata yeni oyuncularda geriden geliyor ve bu gecikme kapanmıyor: §4.7'nin adıyla andığı dört oyuncu (Yoro, Dorgu, Santos, Gabbia) **bir yıl sonra hâlâ eksik**.

##### Veri Vikipedi'de duruyor

Eksik 5.547 oyuncunun İngilizce makalesi çekilip kariyer kutuları ayrıştırıldı:

| Ölçüm                                 |               Sayı |
| ------------------------------------- | -----------------: |
| Kariyer kutusu okunabilen oyuncu      |      5.532 (%99,7) |
| Ham kariyer satırı                    |             25.656 |
| **Evrendeki bir kulübe düşen satır**  | **14.444** (%56,3) |
| En az bir dönem üretecek oyuncu       |          **5.490** |
| `MIN_APPEARANCES` (100) eşiğini geçen |          **1.225** |

Kalan ~4.265 oyuncu istatistik modunun eşiğinin altında kalır ama **ızgara modunu** besler; orada ölçüt dönem, maç değil.

##### Katmanın yaptığı tek yeni iş KEŞİF

Boru hattının geri kalanı zaten yazılmış durumda ve bu, işin boyutunu §10.2'nin tahmininden ("ayrı bir çekim katmanı") küçültüyor:

| Parça                             | Durum                                                  |
| --------------------------------- | ------------------------------------------------------ |
| Kadro şablonundan oyuncu bulma    | **YENİ** — yazılan tek şey                             |
| Makale başlığı → QID              | **YENİ** — `prop=pageprops&ppprop=wikibase_item`       |
| Oyuncu künyesi                    | `playerDetails()` — `P54` şartı yok, keyfi QID listesi |
| Bilgi kutusundan kariyer          | `parseInfoboxSpells()`                                 |
| Kulüp bağlantısı → QID            | `wikipedia-pass` ters indeksi (kural 5 yerinde kalır)  |
| Wikidata'da olmayan dönemi ekleme | `merge-wikipedia` 1. kuralı + `syntheticSpellId()`     |
| Resmî toplamlar                   | `parseCareerTotal()` (§9.2)                            |

**Keşif YALNIZCA İngilizce yapılır.** `{{fs player}}` ailesi tek bir şablon ailesidir ve evrendeki kulüplerin 877/889'unda mevcut; her dilin kendi kadro şablonunu ayrıştırmak gereksiz, çünkü keşfin dilinin doğrulamanın diliyle aynı olması gerekmiyor. Keşif `en`'de yapılır, **kariyer beş dilde doğrulanır**.

##### İkinci kaynak şartı — BR-60 ve dil politikasına istisna

Bu oyuncularda **Wikidata omurgası yoktur**, dolayısıyla BR-42'nin koruduğu şey korunmaz: bugüne kadar her dönemin bir Wikidata dayanağı vardı ve Vikipedi ya onu tamamlıyor ya ona itiraz ediyordu. Leão vandalizmini yakalayan da bu ikilikti. Tek kaynağa dayanan bir dönem, bozulmuş bir bilgi kutusunu doğrudan içeri alır.

Çare projede zaten var: `WikipediaSpell.sites` alanı kaydı **hangi dillerin ürettiğini** saklıyor ve `wikipedia-verdict.ts` "≥2 bağımsız dil" ölçütünü zaten kanıt sayıyor. **BR-60 aynı çıtayı bu yöne uygular.**

**Aşama 2'nin iki kademeli dil politikası bu oyuncular için GEÇERSİZDİR** ve bu bilinçli bir istisnadır. O politikanın gerekçesi "`tr`/`en` makalesi olan oyuncunun ana dil satırlarının %88–96'sı zaten Wikidata'da var" idi; burada Wikidata'da **hiçbir şey yok**, yani gerekçe düşer. Ölçüm de bunu zorunlu kılıyor:

| Senaryo                                   |    Oyuncu (5.490) | Eşiği geçen (1.225) |
| ----------------------------------------- | ----------------: | ------------------: |
| `en` var (keşfin kendisi)                 |      5.490 (%100) |        1.225 (%100) |
| `tr` **ve** `en` birlikte                 |       905 (%16,5) |         272 (%22,2) |
| `en` + en az bir ana dil (`it`/`de`/`fr`) |     3.826 (%69,7) |       1.038 (%84,7) |
| Beş dilden ≥2                             | **3.918 (%71,4)** |   **1.048 (%85,6)** |

Varsayılan politika bırakılsaydı ikinci kaynak oyuncuların yalnızca %16,5'inde sorulurdu. Ana diller keşfedilen oyuncular için **koşulsuz** okunur; maliyeti dil başına ~110 istektir.

##### Kapı önce GÖLGE MODUNDA koşar
...
Bu bir kusur değil, kaynağın şeklinin sonucu.

**KARARA BAĞLANDI — YAPILMAYACAK (23 Ağustos 2026, ürün sahibi).** Bilgi
kutusunda `nationalcaps` alanı **var**, yani teknik olarak okunabilir. Yine de
yazılmıyor: BR-14'ün semantiği (toplam DEĞİL, tek takım için en büyük) o
alandan doğrudan çıkmıyor — U-21 ile A takımı ayrı satırlarda ve ayrımı
yalnızca takım adı taşıyor. Kazanç ise dar: keşfedilen 3.541 oyuncunun büyük
kısmı zaten `MIN_APPEARANCES` eşiğinin altında (%19,4'ü geçiyor), yani millî
maç eklense bile istatistik havuzuna girecek sayı birkaç yüzü aşmaz.

Izgara modundaki kazanç (+8.470 dönem) bu katmanın asıl getirisidir ve o
zaten alınmıştır.
...
> **KARARA BAĞLANDI — YAPILMAYACAK (23 Ağustos 2026, ürün sahibi).** Sayı
> çakışmasında "ilk dil kazanır" kuralı kulüp ADI için doğru (Türkçe bu evrene
> en yakın dil) ama SAYI için keyfîdir: bir dil `0`, üçü `76` diyorsa `0`'ı
> seçmek için bir gerekçe yok. Kural yine de **değiştirilmiyor** — gerekçe
> ölçülen büyüklük: en fazla 397 dönem, yani 414 binin **binde biri**. Kuralı
> değiştirmek dönemlerin tamamını etkiler ve kendi ölçümünü ister; kazanç bu
> maliyeti karşılamıyor.
>
> Yeniden açılırsa doğru biçim şudur ve ölçülmeden uygulanmamalıdır: ad
> seçiminde dil önceliği KALIR, yalnızca maç/gol çakışmasında en büyük değer
> alınır. "Türkçeyi listeden çıkaralım" bir çözüm değildir — Türkçe kulüp adı
> ve yönlendirme eşleştirmesinde en verimli ikinci dildir (%58).

##### Bilinen sınırlar

- **Yalnızca GÜNCEL kadro boşluğunu kapatır.** Tarihî eksikler yerinde kalır; kadro şablonu bugünün kadrosunu yazar.
- **147 kulüpte kadro şablonu yok** (düz tablo kullanıyorlar), 12 kulübün İngilizce makalesi yok. Bu kulüpler keşfin dışındadır.
- **D sınıfı (6.169 kadro yeri) kapanmaz** ve kapatılması hedeflenmiyor: Vikipedi makalesi olmayan oyuncu tanınırlık havuzuna da giremez.
- Ölçümün kendisi **sadeleştirilmiş bir ayrıştırıcıyla** yapıldı; üretim ayrıştırıcısı beş dilli ve daha iyi okuyor (Aşama 2'de ölçüldü). Yukarıdaki sayılar bu yüzden **alt sınırdır**.
...
**Millî takımın ülkesi nasıl bulunuyor.** `P1532` (spor için ülke) TAKIM üzerinde seyrek; ölçüldü: dört takımın yalnızca birinde var. `P17` (ülke) çalışıyor ve İngiltere millî takımını `GB`'ye eşliyor — bu, kulüp/oyuncu kodlamasıyla **tutarlı** (veri kümesindeki en kalabalık uyruk zaten `GB`). Sıra: önce `P1532`, sonra `P17`.

**P1532 OYUNCU üzerinde ikinci kademe oldu (2 Eylül 2026).** Yukarıdaki `P1532`, TAKIMIN ülkesi içindir ve seyrektir; OYUNCUNUN `P1532`'si ("spor için ülke") ise ayrı ve doludur. Gerçek testte ortaya çıkan kusur: millî takımı Wikidata'da OLMAYAN — kıdemli caps'i BR-64 ile Vikipedi bilgi kutusundan dolan — çifte vatandaşlı oyuncularda 1. kademe (Wikidata millî takım) boş kalıyor ve uyruk vatandaşlığa/doğum ülkesine düşüp YANLIŞ çıkıyordu: **Kaan Ayhan → DE, Mert Müldür → AT, Salih Özcan → DE** (üçü de A Millî Takım oyuncusu). Wikidata'da bu üçünün kıdemli Türkiye takımı yok; Kaan'da yalnız gençlik takımları (Almanya U-16/17/18, Türkiye U-21/17), Müldür/Salih'te hiç. Ama oyuncunun `P1532`'si doğru: Salih/Müldür'de tek değer `TR`, Kaan'da `{DE, TR}` (gençlik + kıdemli). Bu yüzden kademe sırası: (1) Wikidata millî takım ülkesi, (2) **`P1532` spor ülkesi**, (3) vatandaşlık, (4) doğum ülkesi. Çift `P1532`'de ayraç **doğum ülkesini eler** (yurt dışında doğup A millîyi değiştirenlerin kalıbı: Kaan spor `{DE,TR}`, doğum DE → `TR`); elemeden sonra tek kalırsa o, hepsi doğum ülkesiyse doğum ülkesi. (Kıdemli takımı DOĞUM ülkesi OLUP yurt dışında gençlik oynamış nadir oyuncuda bu ayraç yanılır; o oyuncuların çoğunda kıdemli takım zaten Wikidata'da olduğundan 1. kademe kazanır ve ayraca hiç düşülmez.) Bu bir veri doğruluğu değişimidir; koşudan sonra kaç uyruğun DEĞİŞTİĞİ ölçülür.
...
- **BR-60 — Wikidata omurgası OLMAYAN oyuncunun dönemi, ≥2 bağımsız dil yazmadıkça yüklenmez.** _(§4.3 Aşama 3 — GÖLGE MODUNDA.)_ Kadro keşfiyle bulunan oyuncunun hiçbir `P54` ifadesi yoktur; kariyerinin tek kaynağı Vikipedi bilgi kutusudur. BR-42 bu oyuncuları **koruyamaz** çünkü o kural iki kaynağın çelişmesini ölçer ve burada çelişecek ikinci kaynak yoktur — bozulmuş bir kutu doğrudan içeri girerdi. Kural, `WikipediaSpell.sites` alanının zaten sakladığı kanıtı kullanır ve çıtayı `wikipedia-verdict.ts`'in kullandığı çıtayla **aynı** tutar: bir dil karantina, iki dil kabul. İki sonucu var. Birincisi, Aşama 2'nin iki kademeli dil politikası bu oyuncular için **geçersizdir** — ana diller koşulsuz okunur, çünkü o politikanın gerekçesi ("ana dil satırları zaten Wikidata'da var") tam olarak burada düşer; ölçüldü, keşfedilenlerin yalnızca **%16,5'inde** Türkçe makale var. İkincisi, kural ilk koşuda **hiçbir şey yazmaz**: dönem başına verimi ölçülmeden eşik açılmaz (§4.3, gölge modu). Kapsam yalnızca keşfedilen oyunculardır; Wikidata dayanağı olan hiçbir dönemin davranışı değişmez.
...
**Kapı neden reddeder, BR-42 neden karar veremez.** BR-42'de bulunan şey "iki kaynağın anlaşamadığı kayıt"tır ve kararı insan vermelidir. Burada bulunan şey aritmetik olarak imkânsızdır — bütünü kapsayan sayı parçasından küçük OKUNAMAZ — ve insana sorulacak bir yanı yoktur; okunan toplam olduğu gibi yazılmaz. (Başta değeri lig sayımıza yükseltmek "uydurma" sayılıp `null` bırakılıyordu; 2 Eylül'de bu gözden geçirildi — lig sayımız bir tahmin değil ÖLÇÜM olduğundan, düşürmek yerine onunla uzlaştırmak §2.7'yi çiğnemez. Aşağı: dar yedek.)

**KÜÇÜK BİR PAY BIRAKILDI — `TALLY_SLACK = 2` (2 Eylül 2026).** Kapı iki toplamı karşılaştırır ve ikisi de İKİ BAĞIMSIZ TOPLAMADIR: bizimki dönem dönem `Spell` değerlerinden, Vikipedi'ninki kariyer tablosunun toplam satırından. Bir-iki birimlik fark aritmetik imkânsızlık değil, kaynak gürültüsüdür (bir dönemin golü bir fazla okunmuş vb.). Gerçek testte çıktı: **Ozan Tufan** kariyer golü 43, bizim dönem toplamımız 44 — bu **1 fark** onun bütün kulüp toplamını (365/43) düşürüp "Resmî maç/gol"da görünmez yapıyordu. Ölçüm (2.395 çatışma, 1 Eylül koşusu): fark 1-2 olan **%14 (331)** — Ozan gibi yakın-eşleşmeler; buna karşılık gerçek kusurlar çok büyük, fark 11+ olan **%69 (1.655)** (Popescu 77↔87 fark 10, Beye 262↔359 fark 97). `2` mutlak payı ilk kümeyi kurtarır, ikinciyi hâlâ reddeder. Pay YÜZDE değil mutlaktır: golde sayılar küçük, yüzde kırılgan olurdu (4↔5 %20 sapar).

**DAR YEDEK — reddedilen toplam DÜŞMEZ, lig sayımızla uzlaştırılır (`reconciled`, 2 Eylül 2026).** Önceki davranış (yukarı) reddedilen toplamı `null` bırakıyordu, ama bu elimizdeki GERÇEK ölçümü (lig sayımız) de çöpe atıyordu. Reddin olağan sebebi Vikipedi toplamının BAYAT olmasıdır: oyuncunun son dönemleri makaleye eklenmemiş, bizim dönem toplamımız o alanda daha günceldir. Gerçek testte çıktı: **Emre Akbaba** en Vikipedi 247/71, bizim lig sayımız 293/78 → `247 < 293` diye düşüyor ve "Resmî maç"ta görünmez kalıyordu; **Yalçın Kayan** 144/13 ↔ 162/13. Artık yazılan değer iki toplamın ALAN-BAZLI max'ıdır: `max(Vikipedi maç, lig maç)`, `max(Vikipedi gol, lig gol)`. Böylece Vikipedi'nin daha büyük olduğu alan — kapsam dışı kariyeri içeren; Popescu maç `642 > 623` — KORUNUR, bayat olduğu alan — Popescu gol `77 < 87` — lig sayımızla düzeltilir. Kör tahmin yok, ölçülmüş iki sayıdan büyüğü alınır. `checkCareerTotals` bunu `reconciled` haritasında döndürür; `accepted` (temiz geçenler) onun alt kümesidir, ETL `reconciled`'i yazar.

**Neden yalnızca çelişenler (2.073), toplamı olmayan herkes (71.783) değil — ölçüldü (2 Eylül).** Dönem toplamı hesaplanabilen 71.783 oyuncu clubApp kazanabilirdi, ama **%97'sinin (69.710) hiç Vikipedi toplamı yok**: onlara lig sayımızı yazmak (a) doğru kişiyi bulduğumuzu doğrulamadan — Vikipedi toplamının VARLIĞI o doğrulamadır — ve (b) kapsam dışı kariyeri kör biçimde eksik sayarak istatistik havuzunu ~4 katına çıkarır, "hangisi daha çok maç" karşılaştırmasını iki farklı anlamla kirletir. Çelişenler (2.073) ise kişinin doğrulandığı, yalnızca sayının bayatladığı güvenli kümedir. Yedek bu yüzden DAR tutuldu; toplamı hiç ayrışmayan güncel oyuncular (parser kapsam sınırı) ayrı bir iştir.
